...
const SignUpPage = () => {
  ...
  const onSubmit = async (e) => {
    ...
    // SAFE FIX: HeroUI wraps standard form events inside e.nativeEvent
    ...
    const { data, error } = await authClient.signUp.email(
      ...
      {
        // This stops the backend from issuing an automatic login response
    ...
    if (data) {
      // CLEAR CACHE FIX: Explicitly sign out right away to drop any
      // temporary client-side session cookies created by the signup callback
      await authClient.signOut();

      toast.success("Registration successful! Please login.");

      // Delay to ensure the navbar re-evaluates the empty auth state and components route cleanly
      setTimeout(() => {
        router.refresh();
      ...
      <Card className="w-full max-w-md p-8 border border-gray-200 dark:border-zinc-800 rounded-xl shadow-md bg-white dark:bg-zinc-950 flex flex-col gap-5">
        <Form onSubmit={onSubmit} className="flex flex-col gap-4">
          ...
          <TextField isRequired name="image" type="url" className="w-full">
            ...
            <Input placeholder="https://your-avatar" className="mt-1" />
            <FieldError className="text-xs text-red-500 mt-1" />
          </TextField>
